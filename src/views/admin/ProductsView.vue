<template>
  <VContainer>
    <VRow class="productsRow">
      <VCol cols="12">
        <h1 class="text-center text-secondary">商品管理</h1>
      </VCol>
      <VDivider></VDivider>
      <VCol cols="12" class="productsTop">
        <VBtn color="secondary" @click="openDialog">新增商品</VBtn>
        <VDataTableServer
          class="productsDataTableServer"
        ...
        >
          ...
          <template #[`item.image`]="{ item }">
            <VImg :src="item.raw.image" width="150px"></VImg>
          </template>
          <template #[`item.sell`]="{ item }">
            <VIcon color="success" icon="mdi-check" v-if="item.raw.sell"></VIcon>
          </template>
          <template #[`item.edit`]="{ item }">
            <VBtn color="error" icon="mdi-pencil" @click="tableEditItem(item.raw)" variant="text"></VBtn>
          </template>
        </VDataTableServer>
      </VCol>
    </VRow>
  </VContainer>
  <VDialog persistent width="500px" scrollable v-model="dialog">
...
// 表單
const categories = ['新生兒', '女童', '男童', '童鞋', '汽車座椅', '電器用品', '學習餐椅', '童書玩具', '彌月禮盒', '其他']
